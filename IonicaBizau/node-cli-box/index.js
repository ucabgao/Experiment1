...
function Box(options, text) {
    // Parse the options
    var self = this, w = options.width || options.w, h = options.height || options.h, defaults = Box.defaults, lines = [];
    ...
    if (text) {
        ...
        var line, textOffsetY;
        var escapeLine = function (line) {
            var escapeCodes = (function () {
                var length = line.text.length, results = [], lineLowerCase = line.text.toLowerCase(), index;
        ...
        if (typeof text === "string") {
            ...
            var stretch = text.stretch || false, autoEOL = text.autoEOL || false, hAlign = text.hAlign || undefined, vAlign = text.vAlign || undefined, splits = text.text.split("\n").map(function (val) {
                return val.trim();
            });
            ...
            if (autoEOL) {
                for (var i = 0; i < splits.length; ++i) {
                    ...
                    if (splits[i].replace(/\u001b\[.*?m/g, "").length > (w - 2)) {
                        ...
                        var actualPlace = 0, escaped = splits[i].replace(/\u001b\[.*?m/g, ""), outsideCode = true, escapedIndex = 0, ii;
                        ...
                        if (ii > 0 && ii < splits[i].length) {
                            var div1 = splits[i].substr(0, actualPlace), div2 = splits[i].slice(actualPlace).trim();
